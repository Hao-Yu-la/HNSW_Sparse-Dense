...
int main() {
    ...
    int max_elements = 1000000;//10000;   // Maximum number of elements, should be known beforehand
    int M = 96;                 // Tightly connected with internal dimensionality of the data
                                // strongly affects the memory consumption
    int ef_construction = 150;  // Controls index search speed/build speed tradeoff
    ...
    // Serialize index
    char hnsw_path[100];
    sprintf(hnsw_path, "random_%dM_dim_%d_ef_%d_M_%d.bin", max_elements/1000000, dim, ef_construction, M);
    ...
    // float sparse_rate = (float)non_zero_sum / ((float)dim * max_elements);
