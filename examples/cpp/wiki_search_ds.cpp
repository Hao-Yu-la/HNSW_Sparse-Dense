...
int main() {
    uint32_t dense_dim = 384;               // Dimension of the elements
    uint32_t max_elements = 10000000;   // Maximum number of elements, should be known beforehand
    ...
    char **attr = new char*[max_elements];
    LoadHybridBinToArray<float, uint64_t>("../../test_data/wiki.hybrid", data, attr, max_elements, dense_dim);
    ...
    // Serialize index
    char hnsw_path[100];
    sprintf(hnsw_path, "wiki_%d_ef_%d_M_%d.bin", max_elements, ef_construction, M);
